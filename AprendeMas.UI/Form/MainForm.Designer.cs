namespace AprendeMas.UI
{
    partial class MainForm
    {
        /// <summary>
        ///  Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        ///  Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        ///  Required method for Designer support - do not modify
        ///  the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            btnStartService = new Button();
            btnStopService = new Button();
            SuspendLayout();
            // 
            // btnStartService
            // 
            btnStartService.Location = new Point(97, 70);
            btnStartService.Name = "btnStartService";
            btnStartService.Size = new Size(134, 23);
            btnStartService.TabIndex = 0;
            btnStartService.Text = "Iniciar dervicio";
            btnStartService.UseVisualStyleBackColor = true;
            btnStartService.Click += btnStartService_Click;
            // 
            // btnStopService
            // 
            btnStopService.Location = new Point(322, 70);
            btnStopService.Name = "btnStopService";
            btnStopService.Size = new Size(143, 23);
            btnStopService.TabIndex = 1;
            btnStopService.Text = "Detener servicio";
            btnStopService.UseVisualStyleBackColor = true;
            btnStopService.Click += btnStopService_Click;
            // 
            // MainForm
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(559, 167);
            Controls.Add(btnStopService);
            Controls.Add(btnStartService);
            Name = "MainForm";
            Text = "Form1";
            ResumeLayout(false);
        }

        #endregion

        private Button btnStartService;
        private Button btnStopService;
    }
}
